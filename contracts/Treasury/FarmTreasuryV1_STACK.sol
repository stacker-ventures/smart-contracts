...
STACK VERSION:
We implement a withdraw fee, and we change the constants to be more applicable for the STACK vault.
Constants:
5,000 STACK will be awarded each month, and there is also a buyback. The fund will be rebalanced monthly, so we will increase the "rebalanceUp" limit to 50%.
There will be a 20% performance fee, however this will be all taken by the treasury. There will not be a reward to the farmer. There will not be a base fee.
Hot wallet holdings will be at 25%, there currently isn't a base strategy for STACK tokens.
The linear unlock period will be 4 weeks.

NEW ADDITIONS TO CODE:
We will implement a 0.5% withdraw fee, this can be changed by governance. Holders will receive a pro-rata share of all withdraw fees.
...
contract FarmTreasuryV1_STACK is ReentrancyGuard, FarmTokenV1 {
	...
	uint256 public performanceToTreasury = 2000; // see constants at top
	uint256 public performanceToFarmer = 0; // see constants at top
	uint256 public baseToTreasury = 0; // see constants at top
	uint256 public baseToFarmer = 0; // see constants at top

	// limits on rebalancing from the farmer, trying to negate errant rebalances
	uint256 public rebalanceUpLimit = 5000; // see constants at top
	...
	uint256 public waitPeriod = 4 weeks; // see constants at top
	...
	uint256 public hotWalletHoldings = 2500; // see constants at top

	uint256 public withdrawFee = 50; // see constants at top
	...
	function setWithdrawFee(uint256 _new) external {
		require(msg.sender == governance, "FARMTREASURYV1: !governance");
		require(_new <= 500, "FARMTREASURYV1: >5% withdraw fee");
	...
	function withdraw(uint256 _amountUnderlying) external nonReentrant {
		...
		// sub the withdraw fee here, note that any funds left in the contract are just automatically assigned to the existing shareholders.
		uint256 _transferAmt = _amountUnderlying;
		if (withdrawFee > 0){
			_transferAmt = _amountUnderlying.mul(max.sub(withdrawFee)).div(max);
		}

		IERC20(underlyingContract).safeTransfer(msg.sender, _transferAmt);
	}
	...
	function _withdraw(uint256 _amountUnderlying) internal {
		...
		emit Transfer(msg.sender, address(0), _amountUnderlying); // burn the shares, no wd fee
		emit Withdraw(msg.sender, _amountUnderlying); // burn the shares, no wd fee here
	}
