...
/*
	...
	MATIC VERSION: Due to limited DEX support, we are making the swap function less strict, and also allowing the XXXRouter's to be changed. Currently they will be set to the
	SushiswapRouter and QuickswapRouter, and as other DEX's start supporting Matic, we can add them in. Uni??

	MATIC: Initially, UniswapRouter == Quickswap Router address... just calling this Uni for consistency with mainnet. Quick == main MATIC DEX

	MATIC: We will not be taking CRVTokens, and removed that logic from the contracts.

	MATIC: We are adding another fee taker for a buyback. Some percentage of revenue will be directed towards a buyback.
*/
...
abstract contract FarmBossV1_MATIC {
	...
	address public UniswapRouter = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // NOTE: is QuickswapRouter, named Uni for consistency (& is largest DEX on Matic ATM)
	address public SushiswapRouter = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; // NOTE: is SushiswapRouter, Matic version
	...
	function setSushiswapRouter(address _new) external {
		require(msg.sender == governance, "FARMBOSSV1: !governance");

		SushiswapRouter = _new;
	}

	function setUniswapRouter(address _new) external {
		require(msg.sender == governance, "FARMBOSSV1: !governance");
	...
	function sellExactTokensForUnderlyingToken(bytes calldata _data, bool _isSushi) external returns (uint[] memory amounts){
		...
		// MATIC NOTE: removing strict checks, as DEX support is very limited
		require(path[path.length.sub(1)] == underlying, "FARMBOSSV1: invalid sell, must swap --> underlying");
