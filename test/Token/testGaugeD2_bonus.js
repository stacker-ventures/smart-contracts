...
const GaugeD2 = artifacts.require("GaugeD2_bonus_configurable");

contract("test GaugeD2_bonus", async (accounts) => {
	...
	async function _initBonusToken(){
		return await MintableToken.new(18, {from: FARMER});
	}

	async function _initGaugeD2(_treasury, _STACK, _bonusToken){
		return await GaugeD2.new(GOVERNANCE, _treasury, _STACK, _bonusToken, {from: FARMER});
	}
	...
	it("should init everything, and check init state", async () => {
		...
		let _bonusToken = await _initBonusToken();
		let _gauge = await _initGaugeD2(_treasury.address, _STACK.address, _bonusToken.address);
		...
		assert.equal(_bonusToken.address, (await _gauge.bonusToken()).toString())
